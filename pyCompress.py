...
def browseInput():
    global input
    global lastLocation
    input = filedialog.askopenfilenames(initialdir = lastLocation + "/", #open at last place used by browser
                                          title = "Select file(s)",
                                          filetypes = ((".mp4", "*.mp4"), ("all files", "*.*")))
    #Update last used location
    lastLocation = path.dirname(input[0])
    # Change label contents
    inputNamesString = ""
    for x in range(len(input)):
        inputNamesString += path.basename(input[x]) + " "
    label_inputPath1.configure(text="File(s) opened: " + inputNamesString)
...
def compressButton():
    global input
    print(f"input {input} length {len(input)}")
    global outputDest
    ...
    for filename in input:
        ...
        if(len(input) > 1):
            outputName = (path.splitext(path.basename(filename))[0].replace(' ', '\ ')+"_Compressed").encode()
        else:
...
#TODO - DOES NOT WORK
def concatenate(input, outputName, outputDest):
    '''
    #run(["ffmpeg", "-i", f"{input}", "-c", "copy", "intermediate1.ts"], shell=True) #make temp file 1
    ...
    run([f"ffmpeg -y -i {input} -c copy -bsf:v h264_mp4toannexb -f mpegts temp1 2> /dev/null & \ "], shell=True)
